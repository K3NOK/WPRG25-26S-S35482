...
$tasks = [
    [
        ...
        'tags' => ['nie_trzy', 'pi'],
        'description' => "mailnij jacek@example.com.\nZajrzyj na stronę http://google.com.\n\n- zrób to\n- zrób tamto\n\n#pilne",
        'end_date' => '2026-05-15'
    ],
    [
        'title' => 'tytul placek',
...
<body>
    ...
    <aside class="sidebar">
        ...
        <?php if (!empty($errors)): ?>
            <div
                style="background-color: #ffcccc; color: #cc0000; padding: 10px; margin: 10px 5px; border-radius: 4px; border: 1px solid #cc0000;">
        ...
        <form method="POST" action="">
            ...
            <input type="text" id="taskTime" name="estimated_minutes" value="<?= htmlspecialchars($inEstimated) ?>"
                required>

            <label for="taskEndDate">Data końcowa (RRRR-MM-DD)</label>
            <input type="text" id="taskEndDate" name="end_date" placeholder="np. 2026-05-15"
                value="<?= htmlspecialchars($inEndDate) ?>">

            <label for="taskDescription">Opis zadania</label>
            <textarea id="taskDescription" name="description" rows="4"
                style="resize:vertical;"><?= htmlspecialchars($inDescription) ?></textarea>

            <label for="taskTags">Tagi (oddzielone spacją)</label>
            <input type="text" id="taskTags" name="tags" placeholder="np. pilne praca trudne"
                value="<?= htmlspecialchars($inTags) ?>">
    ...
    <main class="main-shii">
        ...
        <div class="search-bar"
            style="background: #dedede; padding: 15px; margin-bottom: 20px; border-radius: 8px; border: 1px solid #626262;">
            <form method="GET" action="" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;">
                <div>
                    <label style="display:block; font-weight:bold; font-size:14px; margin-bottom:5px;">Wyszukaj
                        (Regex)</label>
                    <input type="text" name="search_regex" value="<?= htmlspecialchars($searchRegex) ?>"
                        placeholder="np. ^tytul" style="padding: 6px; border: 1px solid #ccc; border-radius: 4px;">
                </div>
                <div>
                    <label style="display:block; font-weight:bold; font-size:14px; margin-bottom:5px;">Tag</label>
                    <input type="text" name="search_tag" value="<?= htmlspecialchars($searchTag) ?>"
                        placeholder="np. pilne" style="padding: 6px; border: 1px solid #ccc; border-radius: 4px;">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="search_status" style="padding: 6px; border: 1px solid #ccc; border-radius: 4px;">
                        <option value="">Wszystkie</option>
                        <option value="do zrobienia" <?= $searchStatus === 'do zrobienia' ? 'selected' : '' ?>>Do zrobienia
                        </option>
                        <option value="w trakcie" <?= $searchStatus === 'w trakcie' ? 'selected' : '' ?>>W trakcie</option>
                        <option value="zakończone" <?= $searchStatus === 'zakończone' ? 'selected' : '' ?>>Zakończone
                        </option>
                    </select>
                </div>
                <div>
                    <button type="submit"
                        style="padding: 7px 15px; background: #3f2929; color: white; border: none; cursor: pointer; border-radius: 4px; font-weight: bold;">Szukaj
                        / Filtruj</button>
                    <a href="index.php"
                        style="display: inline-block; padding: 7px 15px; background: #626262; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; margin-left: 5px;">Wyczyść</a>
                </div>
            </form>
        </div>
        <div class="stats-bar">
